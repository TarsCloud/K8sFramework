...
void K8SListWatchSession::prepare() {
    _waitCacheSyncSize.fetch_add(1);
...
void K8SListWatchSession::afterReadListResponse(asio::error_code ec, size_t bytes_transferred) {
    ...
    if (pLimitContinue == nullptr) {
        ...
        _waitCacheSyncSize.fetch_sub(1);
        doWatchRequest();
        return;
    }
...
void K8SListWatchSession::afterWatchRequest(asio::error_code ec, std::size_t bytes_transferred) {
    if (ec) {
        return fail(ec, __FUNCTION__);
    }
    clearResponseState();
...
bool K8SListWatchSession::handlerK8SWatchEvent(const rapidjson::Document &document) {
    ...
    constexpr char MODIFIEDTypeValue[] = "MODIFIED";
    ...
    } else if (strcmp(MODIFIEDTypeValue, pType) == 0) {
        what = K8SEventTypeModified;
