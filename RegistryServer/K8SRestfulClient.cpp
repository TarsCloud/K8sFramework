...
public:
    explicit K8SRestClientSession(asio::io_context &ioContext) : sslContext_(
...
void K8SRestfulClient::postTask(K8SRestfulTaskType type, const std::string &url, const std::string &body) {
    std::string content{};
    switch (type) {
        case Patch: {
            ...
            [this, contentPtr] {
                ...
                if (!workSuccess) {
                    ...
                    for (size_t i = 0; i < RetryTimes; ++i) {
                        ...
                        sessionPtr_ = std::make_shared<K8SRestClientSession>(ioContext_);
...
void K8SRestfulClient::run() {
    sessionPtr_ = std::make_shared<K8SRestClientSession>(ioContext_);
    asio::io_context::work work(ioContext_);
    ioContext_.run();
}

std::string K8SRestfulClient::buildPostContent(const std::string &url, const std::string &body) {
...
std::string K8SRestfulClient::buildPatchContent(const std::string &url, const std::string &body) {
    std::ostringstream strStream;
...
std::string K8SRestfulClient::buildDeleteContent(const std::string &url) {
    std::ostringstream strStream;
...
std::string K8SRestfulClient::buildSMPatchContent(const string &url, const string &body) {
    std::ostringstream strStream;
    strStream << "PATCH " << url << " HTTP/1.1\r\n";
    strStream << "Authorization: Bearer " << K8SRuntimeParams::interface().bindToken() << "\r\n";
    strStream << "Host: " << K8SRuntimeParams::interface().apiServerHost() << ":"
