...
//    //启动脚本处理
//    _sStartScript = TC_Common::trim(descriptor.startScript);
//    if (!_sStartScript.empty() && !TC_File::isAbsolute(_sStartScript)) {
//        _sStartScript = _sServerBaseDir + _sStartScript;
//    }
//
//    //停止脚本处理
//    _sStopScript = TC_Common::trim(descriptor.stopScript);
//    if (!_sStopScript.empty() && !TC_File::isAbsolute(_sStopScript)) {
//        _sStopScript = _sServerBaseDir + _sStopScript;
//    }
//
//    //监控脚本处理
//    _sMonitorScript = TC_Common::trim(descriptor.monitorScript);
//    if (!_sMonitorScript.empty() && !TC_File::isAbsolute(_sMonitorScript)) {
//        _sMonitorScript = _sServerBaseDir + _sMonitorScript;
//    }
...
int ServerObject::MetaData::updateConfFromDescriptor(const ServerDescriptor &descriptor) {
    try {
        ...
        _vAdapter.clear();
        for (auto itAdapters = descriptor.adapters.rbegin(); itAdapters != descriptor.adapters.rend(); itAdapters++) {
            TLOGINFO(FILE_FUN << "get adapter " << itAdapters->first << endl);
            _vAdapter.push_back(itAdapters->first);
            string endpointString = itAdapters->second.endpoint;
            constexpr char LOCAL_IP_STR[] = "localip";
            auto pos = endpointString.find(LOCAL_IP_STR);
            if (pos != std::string::npos) {
                endpointString.replace(pos, sizeof(LOCAL_IP_STR) - 1, container_detail::podIp);
            }
            m["endpoint"] = endpointString;
        ...
        m["localip"] = container_detail::podIp;
        ...
        for (const auto &item:m) {
            sProfile = TC_Common::replace(sProfile, "${" + item.first + "}", TC_ConfigDomain::reverse_parse(item.second));
        }
...
ServerObject::ProcessStatus ServerObject::_doStart(std::string &result) {
    ...
    do {
        try {
            ...
            int res = registryProxy->getServers(_metaData._sServerApp, _metaData._sServerName, descriptor);
...
void ServerObject::updateKeepAliveTime(const string &adapter) {
    ...
    if (_runtimeData.presentState == Activating) {
        _runtimeData.presentState = Active;
        uploadState(_runtimeData.settingState, Active);
    }
...
void ServerObject::updateKeepActiving() {
    ...
    if (_runtimeData.presentState != Activating) {
        _runtimeData.presentState = Activating;
        uploadState(_runtimeData.settingState, Activating);
    }
...
//    auto isScriptFileFunction = [this](const std::string &sFileName) {
//        if (TC_Common::lower(TC_File::extractFileExt(sFileName)) == "sh") //.sh文件为脚本
//        {
//            return true;
//        }
//        if (TC_File::extractFileName(_metaData._sStartScript) == sFileName) {
//            return true;
//        }
//        if (TC_File::extractFileName(_metaData._sStopScript) == sFileName) {
//            return true;
//        }
//        return TC_File::extractFileName(_metaData._sMonitorScript) == sFileName;
//    };
//
//
//    bool bIsScriptFile = isScriptFileFunction(sFileName);
//    if (bIsScriptFile) {
...
void ServerObject::uploadState() {
    try {
        ...
        ptr->updateState(container_detail::podName, etos(_runtimeData.settingState), etos(_runtimeData.presentState));
    } catch (const exception &e) {
        LOG->debug() << "uploadState exception : " << e.what() << endl;
    }
}
...
void ServerObject::uploadState(tars::ServerState settingState, tars::ServerState presentState) {
    TimerTaskQueue::instance().pushTimerTask([settingState, presentState] {
        try {
            ...
            LOG->debug() << "Update State " << info.settingState << "|" << info.presentState << endl;
            ptr->updateState(container_detail::podName, etos(settingState), etos(presentState));
        } catch (const exception &e) {
            LOG->debug() << "uploadState exception : " << e.what() << endl;
        }
    }, 0);
