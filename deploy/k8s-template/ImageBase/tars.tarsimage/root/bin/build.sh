...
function Check_Cpp_Lib() {
  ...
  LddRes=$(ldd ${BuildServerExeFile})

  if [[ "${LddRes}" == "" ]]; then
    Record_Task_Error_Status "程序本身不存在"
    exit 255
  fi

  chmod +x ${BuildServerExeFile}

  if [[ "${LddRes}" =~ "not a dynamic executable" ]]; then
    Record_Task_Working_Status "静态编译无需其他依赖"
    return
  fi
  ...
  Record_Task_Working_Status "检测程序依赖文件成功"
}
