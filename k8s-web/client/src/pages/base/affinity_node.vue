<template>
  <div class="page_base_affinity">
    <div style="float: right">
      <let-button size="small" theme="primary" @click="addItem">{{$t('filter.btn.addNode')}}</let-button>
    </div>
    ...
    <let-table ref="table" :data="items" :empty-msg="$t('common.nodata')">
      ...
      <let-table-column :title="$t('filter.title.app')" prop="ServerApp">
        <template slot-scope="scope">
          <div class="table_con">
      ...
      <let-form ref="detailForm" v-if="detailModal.model" itemWidth="700px">
        ...
        <let-form-item :label="$t('filter.title.app')">
          ...
          <div class="checkbox_list">
            <let-checkbox class="checkbox_item" v-for="d in AppList" :key="d.AppName" :value="checkBoxAddList.indexOf(d.AppName) > -1" @change="checkedChange(d.AppName)">{{ d.AppName }}</let-checkbox>
          </div>
          <!-- <let-select
            size="small"
...
export default {
    ...
    isCheckedAll() {
      ...
      if(isCheckedAll) {
        this.AppList.forEach(item => {
          if(this.checkBoxAddList.indexOf(item.AppName) === -1){
            this.checkBoxAddList.push(item.AppName)
          }
          this.checkBoxDelList = []
        })
      }else{
        this.AppList.forEach(item => {
          if(this.checkBoxDelList.indexOf(item.AppName) === -1){
            this.checkBoxDelList.push(item.AppName)
          }
  ...
  methods: {
    ...
    fetchData() {
      ...
      return this.$ajax.getJSON('/server/api/affinity_list_node', this.query).then((data) => {
        loading.hide();
    ...
    editItem(d) {
      this.checkBoxAddList = d.ServerApp || []
    ...
    addServerItem() {
      if (this.$refs.detailForm.validate()) {
        ...
        const url = '/server/api/affinity_add_server';

        const loading = this.$Loading.show();

        if(this.checkBoxDelList && this.checkBoxDelList.length > 0){
          this.$ajax.postJSON('/server/api/affinity_del_server', {
            NodeName: model.NodeName,
            ServerApp: this.checkBoxDelList,
          })
        }

        this.$ajax.postJSON(url, {
          NodeName: model.NodeName,
          ServerApp: this.checkBoxAddList,
        }).then(() => {
          loading.hide();
    ...
    removeItem(d) {
      this.$confirm(this.$t('template.delete.confirmTips'), this.$t('common.alert')).then(() => {
        const loading = this.$Loading.show();
        this.$ajax.getJSON('/server/api/affinity_del_server', { AppName: d.AppName }).then(() => {
          loading.hide();
          this.fetchData().then(() => {
            this.$tip.success(this.$t('common.success'));
          });
