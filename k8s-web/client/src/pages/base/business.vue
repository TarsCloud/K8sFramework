...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      return this.$ajax.getJSON('/server/api/business_select', Object.assign(this.query, {
    ...
    saveItem() {
      if (this.$refs.detailForm.validate()) {
        ...
        const url = this.detailModal.isNew ? '/server/api/business_create' : '/server/api/business_update';
    ...
    removeItem(d) {
      this.$confirm(this.$t('template.delete.confirmTips'), this.$t('common.alert')).then(() => {
        const loading = this.$Loading.show();
        this.$ajax.getJSON('/server/api/business_delete', { BusinessName: d.BusinessName }).then(() => {
          loading.hide();
          this.fetchData().then(() => {
            this.$tip.success(this.$t('common.success'));
          });
    ...
    getAppList() {
      this.$ajax.getJSON('/server/api/application_select', {
        isAll: true,
      }).then((data) => {
    ...
    appListItem(d) {
      this.getAppList()
      const loading = this.$refs.table.$loading.show();
      return this.$ajax.postJSON('/server/api/business_list_app', {
        BusinessName: [d.BusinessName],
      }).then((data) => {
        loading.hide();
        this.appListEditItem(data)
      }).catch((err) => {
        loading.hide();
        this.$tip.error(`${this.$t('common.error')}: ${err.message || err.err_msg}`);
      });
    ...
    appListSaveItem() {
      if (this.$refs.AppForm.validate()) {
        ...
        const url = this.AppModal.isNew ? '/server/api/business_add_app' : '/server/api/business_add_app';
