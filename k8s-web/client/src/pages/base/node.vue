  ...
  <div class="page_base_node">
    ...
    <let-form inline itemWidth="200px" @submit.native.prevent="search">
      ...
      <!-- <let-form-item :label="$t('filter.title.mark')">
        <let-input size="small" v-model="query.NodeMark"></let-input>
      </let-form-item> -->
      <let-form-item>
    ...
    <let-table ref="table" :data="items" :empty-msg="$t('common.nodata')">
      <let-table-column width="60px">
        <template slot="head" slot-scope="props">
          <let-checkbox v-model="isTableCheckedAll"></let-checkbox>
        </template>
        <template slot-scope="scope">
          <let-checkbox v-model="scope.row.isChecked" @change="tableCheckChange(scope.row)"></let-checkbox>
        </template>
      </let-table-column>
      ...
      <let-table-column :title="$t('filter.title.nodeAddress')">
        <template slot-scope="scope">
          <div v-for="item in scope.row.NodeAddress" :key="item.NodeName">{{ item }}</div>
        </template>
      </let-table-column>
      <!-- <let-table-column :title="$t('filter.title.nodeInfo')" width="40%">
        <template slot-scope="scope">
          <div style="padding:0 10px;text-align:justify">{{ scope.row.NodInfo }}</div>
        </template>
      </let-table-column> -->
    ...
    >
      <let-form ref="detailForm" v-if="detailModal.model" itemWidth="700px">
        ...
        <!-- <let-form-item :label="$t('filter.title.mark')" v-if="!detailModal.isNew">
          <let-input
            size="small"
            v-model="detailModal.model.NodeMark"
          ></let-input>
        </let-form-item> -->
      </let-form>
    </let-modal>
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      return this.$ajax.getJSON('/server/api/node_select', Object.assign(this.query, {
    ...
    startData(val) {
      ...
      this.$ajax.postJSON('/server/api/node_start', {
        NodeName,
      }).then(() => {
    ...
    stopData(val) {
      ...
      this.$ajax.postJSON('/server/api/node_stop', {
        NodeName,
      }).then(() => {
        this.$tip.success(this.$t('common.success'));
        this.fetchData();
    ...
    saveItem() {
      if (this.$refs.detailForm.validate()) {
        ...
        let url = '/server/api/node_update'
        let NodeName = model.NodeName
        if(this.detailModal.isNew){
          url = '/server/api/node_add'
    ...
    removeItem(d) {
      this.$confirm(this.$t('template.delete.confirmTips'), this.$t('common.alert')).then(() => {
        const loading = this.$Loading.show();
        this.$ajax.getJSON('/server/api/node_delete', { NodeName: d.NodeName }).then(() => {
          loading.hide();
          this.fetchData().then(() => {
            this.$tip.success(this.$t('common.success'));
          });
