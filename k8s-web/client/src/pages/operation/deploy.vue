...
export default {
  ...
  mounted() {
    ...
    //   this.templates = data;
    //   this.model.ServerTemplate = data[0]
    // }).catch((err) => {
  ...
  methods: {
    ...
    deploy() {
      if (this.$refs.form.validate()) {
        ...
        this.$confirm(this.$t('deployService.form.deployServiceTip'), this.$t('common.alert')).then(() => {
          ...
          this.$ajax.postJSON('/server/api/deploy_create', data).then((data) => {
            loading.hide();
    ...
    getAppList() {
      this.$ajax.getJSON('/server/api/application_select', { isAll: true }).then((data) => {
