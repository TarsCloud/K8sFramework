    ...
    <let-table ref="table" :data="items" :empty-msg="$t('common.nodata')">
      ...
      <let-table-column :title="$t('serviceApproval.ApprovalResult')" prop="ApprovalResult">
        <template slot-scope="scope">
          <span class="success" v-if="`${scope.row.ApprovalResult}` === 'true'">通过</span>
          <span class="warn" v-else-if="`${scope.row.ApprovalResult}` === 'false'">驳回</span>
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      return this.$ajax.getJSON('/server/api/approval_select', {
