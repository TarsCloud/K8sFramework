  ...
  <div class="page_server_config">
    ...
    <wrapper v-if="configList" ref="configListLoading">
      ...
      <let-table :data="configList" :title="$t('cfg.title.a')" :empty-msg="$t('common.nodata')">
        ...
        <let-table-column :title="$t('serverList.table.th.service')" prop="ServerId"></let-table-column>
        <let-table-column :title="$t('cfg.btn.fileName')" prop="ConfigName"></let-table-column>
        ...
        <!-- <let-table-column :title="$t('operate.operates')" width="320px">
          <template slot-scope="scope">
            <let-table-operation @click="nodeConfigAdd(scope.row)">{{$t('filter.btn.addNode')}}</let-table-operation>
            <let-table-operation @click="changeConfig(scope.row, 'configList')">{{$t('operate.update')}}</let-table-operation>
            <let-table-operation @click="deleteConfig(scope.row.ConfigId)">{{$t('operate.delete')}}</let-table-operation>
      ...
      <let-table :data="nodeConfigList" :title="$t('cfg.title.c')" :empty-msg="$t('common.nodata')">
        ...
        <!-- <let-table-column :title="$t('operate.operates')" width="260px"> -->
          <!-- <template slot-scope="scope"> -->
            ...
            <!-- <let-table-operation @click="showMergedDetail(scope.row.ConfigId)">{{$t('cfg.table.viewMerge')}}</let-table-operation> -->
            <!-- <let-table-operation @click="deleteConfig(scope.row.ConfigId)">{{$t('operate.delete')}}</let-table-operation> -->
            <!-- <let-table-operation @click="showDetail(scope.row)">{{$t('cfg.table.viewIpContent')}}</let-table-operation> -->
            <!-- <let-table-operation @click="showHistory(scope.row)">{{$t('pub.btn.history')}}</let-table-operation> -->
            <!-- <let-table-operation @click="handleRefFiles(scope.row.ConfigId)">{{$t('cfg.table.mangeRefFile')}}</let-table-operation> -->
          <!-- </template> -->
        <!-- </let-table-column> -->
      </let-table>
    </wrapper>
    ...
    <let-modal
      ...
      <let-table
        ...
        <let-table-column width="40px">
          <template slot-scope="scope">
            <let-radio v-model="backConfigId" :label="scope.row.HistoryId">&nbsp;</let-radio>
          </template>
        </let-table-column>
        ...
        <let-table-column :title="$t('cfg.btn.reason')" prop="ConfigMark">
          <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    getConfigList(query) {
      ...
      this.$ajax.getJSON('/server/api/server_config_select', {
        ServerId,
      }).then((data) => {
        loading.hide();
        this.configList = [];
    ...
    updateConfigFile() {
      if (this.$refs.configForm.validate()) {
        ...
        if (this.configModal.isNew) {
          ...
          this.$ajax.postJSON('/server/api/server_config_create', query).then((res) => {
            loading.hide();
        ...
        // 修改
          ...
          this.$ajax.postJSON('/server/api/server_config_update', {
    ...
    backConfig() {
      ...
      const loading = this.$Loading.show();
      this.$ajax.postJSON('/server/api/server_config_history_back', {
    ...
    deleteConfig(data, key, val) {
      ...
      this.$confirm(this.$t('cfg.msg.confirmCfg'), this.$t('common.alert')).then(() => {
        const loading = this.$Loading.show();
        this.$ajax.getJSON('/server/api/server_config_delete', {
    ...
    getNodeConfigList() {
      ...
      this.$ajax.getJSON('/server/api/server_config_select', query).then((data) => {
        // loading.hide();
    ...
    showHistory(data, key, val) {
      ...
      this.$ajax.getJSON('/server/api/server_config_history_select', {
        ConfigId: newData[0].ConfigId,
      }).then((data) => {
        loading.hide();
        this.detailModal.model = {
