      ...
      <div class="btabs_con">
        <div class="btabs_con_item" v-for="item in BTabs" :key="item.id" v-show="item.id === treeid">
          <let-tabs @click="clickTab" :activekey="item.path">
            ...
            <let-tab-pane :tabkey="base + '/history'" :tab="$t('header.tab.tab7')"></let-tab-pane>
...
export default {
  ...
  watch: {
    'treeid': function (treeid) { // eslint-disable-line
  ...
  methods: {
    ...
    getServerData() {
      ...
      // serverData.level = treeArr.length
...
.page_server {
  ...
  .left-tree {
    ...
    li > .pointer:first{display:inline-block;width:18px;}
  ...
  /*服务状态*/
  .status-active, .status-inactive, .status-off, .status-activeing, .status-deactivating, .unknown {
    display: flex;
    align-items: center;
  ...
  .btabs_con_item{display:flex;flex:1;flex-flow:column;overflow:hidden;position:relative;}
}
</style>
